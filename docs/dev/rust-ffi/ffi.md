...
The `extern "C"` makes this function adhere to the C calling convention, as discussed below in "[Foreign Calling Conventions](ffi.html#foreign-calling-conventions)".
...
That's it!
For more realistic example, check the [`cbindgen`](https://github.com/eqrion/cbindgen).
...
Please note that [`catch_unwind`](../std/panic/fn.catch_unwind.html) will only catch unwinding panics, not
those that abort the process. See the documentation of [`catch_unwind`](../std/panic/fn.catch_unwind.html)
