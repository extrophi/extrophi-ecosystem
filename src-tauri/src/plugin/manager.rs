...
use super::{PluginError, TranscriptionPlugin, AudioData, Transcript};
...
impl PluginManager {
    ...
    pub fn register(&mut self, plugin: Box<dyn TranscriptionPlugin>) -> PluginResult<()> {
        ...
        self.plugins.insert(name.clone(), Arc::new(Mutex::new(plugin)));
    ...
    pub fn get_active(&self) -> PluginResult<Arc<Mutex<Box<dyn TranscriptionPlugin>>>> {
        let name = self.active_plugin.as_ref()
